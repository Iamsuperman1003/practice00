...
<form method="POST" action="${pageContext.request.contextPath}/emp/add" name="form1">
    <table>
        <tr>
            <td>員工姓名:</td>
            <td>
            ...
            <td>
                <select size="1" name="deptno">
                    <c:forEach var="deptDO" items="${deptDOs}">
                        <option value="${deptDO.deptno}"
                                <c:if test="${empDO != null && empDO.deptno == deptDO.deptno}">selected</c:if>
                        >${deptDO.dname}</option>
                    </c:forEach>
                </select>
            </td>
